...
def _dedupe_bundles(bundles: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for b in bundles:
        # each bundle is expected to contain `references` list
...
def cmd_dedupe_canonical(args):
    ...
    if not isinstance(data, list):
        # try to interpret as list of bundles inside a dict
        data = [data]
...
def cmd_zotero_citation(args):
    ...
    # If this is a canonical bundle, extract the reference
